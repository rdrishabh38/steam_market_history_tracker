...
class DataProcessor:
    ...
    def _parse_html_results(self, html_content: str) -> list:
        ...
        for row in rows:
            ...
            transactions.append({
                "Acted On Date": date_divs[0].text.strip() if len(date_divs) > 0 else "N/A",
                "Listed On Date": date_divs[1].text.strip() if len(date_divs) > 1 else "N/A",
                "Item Name": item_name_span.text.strip() if item_name_span else "N/A",
                "Type": "Purchase" if action_char == '+' else "Sale",
                "Price": price
            })
        return transactions
    ...
    def run(self):
        ...
        if not os.path.exists(self.data_dir):
            raise FileNotFoundError(f"The data directory '{self.data_dir}' does not exist. Please run the downloader first.")
        ...
        self.log_queue.put((f"Successfully parsed a total of {len(all_transactions)} transactions.", "white"))
...
def run_processing(log_queue):
    try:
        processor = DataProcessor(log_queue)
        processor.run()
        return True
